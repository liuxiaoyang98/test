...
$pdo=new pdoC;
    echo "<div class='row' id='upper'>";
        if($_SESSION[login]==1){
            ...
            echo "<div class='col-md-8 text-right'><h4>游客您好 请您<a href='/lxy/vote3.0/users/reg.php'>注册</a>或者<a href='/lxy/vote2.0/users/login.php'>登录</a></h4></div>";
        }
    ...
    $suoyin.="<li role = 'presentation'><a href='/lxy/vote2.0/index.php'>首页</a></li>";
    ...
    foreach ($vname as $key => $value) {
        $suoyin.="<li><a href='/lxy/vote2.0/voting/index.php?vid=".$value[0]."'>".$value[1]."</a></li>";
    }
...
function right_votes(){
    ...
    foreach ($vname as $key => $value) {
        $right.="<li><a href='/lxy/vote2.0/voting/index.php?vid=".$value[0]."'>".$value[1]	."</a></li>";
    }
    $right.="</ul></div>";
    echo $right;
}