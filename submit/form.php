...
$time=mysql_fetch_assoc(mysql_query("SELECT * FROM `votes` WHERE `v_id` = '$_GET[vid]'"))[v_time];
$now=date("y-m-d");
...
$ip1=$_SERVER["REMOTE_ADDR"];
    if($_SESSION[login]==1){//已登录
        ...
        $exeres2=$dbh->prepare($query2)->execute($_SESSION[user]);
        $uid=PDOStatement::fetch($exeres2)["id"];
        $query3="SELECT `v_id` FROM `usersVote` WHERE `u_id` = '$uid' AND `v_id`='$_GET[vid]'";
        if(mysql_num_rows(mysql_query($query3))){//userVote有id也有参加投票的记录 则显示已经投票
            echo "请不要重复投票!";
        }else{ //userVote表中没有参加完投票的记录
            $query4="INSERT INTO `vote`.`usersVote` (`u_id`, `v_id`, `votes`) VALUES ('$uid', '$_GET[vid]', '$vote')";
            mysql_query($query1);
            mysql_query($query4);//插入记录
            echo "投票成功";
        }
    }else{//user表中没有ip
